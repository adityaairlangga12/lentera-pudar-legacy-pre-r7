...
# Lentera Godot MCP Bridge Plugin for Godot 4.7
# Runs directly as an EditorPlugin with native _process loop for WebSocket communication
...
func _enter_tree() -> void:
	print("[Lentera Godot Bridge] Plugin active, connecting to MCP on port ", _ws_port)
	connect_to_mcp()

func _exit_tree() -> void:
	print("[Lentera Godot Bridge] Plugin deactivating...")
	_client.close()
	_connected = false
